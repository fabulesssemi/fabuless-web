...
export function SupplyChainWeb() {
  ...
  const activeEdges = active
    ? edges.filter((e) => e.from === active || e.to === active)
    : [];
      ...
      {activeNode && (
        <div className="border-t border-[#DDDBD2] px-4 py-3">
          ...
          <div className="mt-2 flex flex-wrap gap-1.5">
            {activeEdges.map((e, i) => {
              const other = nodeById[e.from === active ? e.to : e.from];
              const dir = e.from === active ? "→" : "←";
              return (
                <span
                  key={i}
                  className={`border bg-white px-2 py-0.5 text-[11px] ${
                    e.critical
                      ? "border-red-300 text-red-700"
                      : "border-gray-300 text-gray-600"
                  }`}
                >
                  {dir} {other?.name} · {e.label}
                </span>
              );
            })}
          </div>

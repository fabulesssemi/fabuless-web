...
const STOCK_EFFECTS: Record<string, Record<string, StockEffect>> = {
  "taiwan-conflict": {
    tsmc:             { dir: "down",  note: "Baker: TSMC capacity is the single most important variable in AI — if it goes offline, everything stops" },
    nvidia:           { dir: "down",  note: "Baker: Jensen has no formal TSMC contract — handshake arrangements don't survive a military conflict" },
    amd:              { dir: "down",  note: "Baker: AMD always runs a step behind on capacity — without fab access, there's no recovery path" },
    broadcom:         { dir: "down",  note: "Baker: Broadcom is the go-to ASIC supplier for every major hyperscaler — all of that pipeline goes to zero" },
    ...
    asml:             { dir: "mixed", note: "Baker: the best semiconductor talent clusters at TSMC and Taiwan — short-term crash but long-term rebuild demand is real" },
    amat:             { dir: "down",  note: "Baker: Applied Materials had key teams at TSMC — Taiwan tool revenue disappears instantly" },
    ...
    arm:              { dir: "down",  note: "Circuit: ARM had been going parabolic on the AI CPU narrative — conflict zeroes the royalty stream" },
    ...
    microsoft:        { dir: "down",  note: "Patel: Azure runs OpenAI at roughly 2GW of compute — all TSMC-dependent, all stranded" },
    google:           { dir: "down",  note: "Patel: TPUs account for the majority of Google and Anthropic training infrastructure — all TSMC-fabbed" },
    amazon:           { dir: "down",  note: "Baker: Trainium is the real ASIC challenger to Nvidia — conflict strands the entire bet" },
    meta:             { dir: "down",  note: "GPU buildout stops; Llama training runs on TSMC-fabbed chips — timeline becomes indefinite" },
    openai:           { dir: "down",  note: "Patel: OpenAI runs roughly 2GW of compute, all TSMC-dependent — costs spike to unaffordable levels" },
    oracle:           { dir: "down",  note: "Stargate is a $100B+ bet on TSMC-fabbed chips — becomes a stranded asset overnight" },
    coreweave:        { dir: "down",  note: "Circuit: Nvidia essentially built the neocloud business model — without TSMC chips, that model collapses" },
    xai:              { dir: "down",  note: "Baker: xAI built Colossus remarkably fast, but it runs entirely on Nvidia/TSMC silicon — expansion halts" },
    anthropic:        { dir: "down",  note: "Patel: Anthropic runs 2-2.5GW on Google TPUs — all TSMC-fabbed; roadmap freezes without new chips" },
  },
  "china-export-controls": {
    asml:             { dir: "down",  note: "Patel: ASML only makes roughly 70 EUV tools per year — a China DUV ban removes 30% of their addressable market" },
    ...
    nvidia:           { dir: "up",    note: "Circuit: the H20 revenue hit has already been absorbed — tighter controls on Chinese AI chips mostly hobble domestic competitors" },
    ...
    "samsung-foundry":{ dir: "up",    note: "Baker: China's fab technology is roughly four years behind and falling further — Samsung gains relative share as Chinese fabs stall" },
  ...
  "hbm-shortage": {
    skhynix:          { dir: "up",    note: "Baker: HBM actually costs more than the TSMC node in Nvidia's bill of materials — SK Hynix holds maximum pricing power in a shortage" },
    micron:           { dir: "up",    note: "Baker: DRAM companies trade at mid-single digit multiples while semiconductor equipment trades at 40x — that gap closes in a shortage" },
    "samsung-memory": { dir: "down",  note: "Patel: Samsung is effectively out of the picture for Rubin HBM4 — the qualification lag gets exposed publicly in a shortage" },
    nvidia:           { dir: "down",  note: "Patel: CoWoS packaging is already the gating constraint on Blackwell — an HBM shortage stacks directly on top; revenue miss likely" },
    amd:              { dir: "down",  note: "Patel: AMD is running pretty mid at single-digit GPU share — HBM shortage hits them alongside Nvidia with no way out" },
    lrcx:             { dir: "up",    note: "Circuit: the memory run was already accelerating — an HBM shortage drives an even faster investment cycle in HBM etch capacity" },
    ...
    openai:           { dir: "down",  note: "Patel: OpenAI is at 2GW and targeting 10GW — any GPU delivery timing risk slips the entire compute buildout plan" },
    xai:              { dir: "down",  note: "Baker: xAI gets access to GPUs ahead of most others — an HBM shortage narrows that structural timing advantage" },
    coreweave:        { dir: "down",  note: "Circuit: Nvidia essentially created the neocloud market — a GPU supply gap from HBM shortage breaks the business model" },
    microsoft:        { dir: "down",  note: "Azure GPU expansion slows; Patel: OpenAI and Anthropic together consume roughly 30% of all Nvidia output" },
    amazon:           { dir: "down",  note: "Baker: Trainium 3 is the real long-term challenger to Nvidia — HBM shortage delays exactly the ramp that matters most" },
  },
  "cowos-bottleneck": {
    tsmc:             { dir: "up",    note: "Patel: CoWoS is the actual gating constraint on the entire Blackwell ramp — TSMC holds all the leverage on AI chip delivery" },
    nvidia:           { dir: "down",  note: "Patel: the real bottleneck is CoWoS packaging, not the TSMC node itself — GPU shipment delays translate directly to revenue misses" },
    ...
    ase:              { dir: "up",    note: "Overflow demand flows to OSATs; Circuit: TSMC supply was constrained all through 2026 — ASE gains real AI packaging share" },
    ...
    amazon:           { dir: "down",  note: "Baker: Trainium is the most credible challenger to Nvidia — the CoWoS bottleneck delays exactly this ramp" },
    ...
    openai:           { dir: "down",  note: "Patel: each gigawatt of data center costs $10-15B — a packaging bottleneck delays every gigawatt in the buildout" },
    ...
    coreweave:        { dir: "down",  note: "Circuit: the neocloud model depends entirely on adding GPU capacity — can't do that without packaged chips" },
  },
  "n3-crunch": {
    tsmc:             { dir: "up",    note: "Baker: TSMC capacity decisions are the most important single variable in AI — N3 scarcity translates directly into pricing power" },
    nvidia:           { dir: "down",  note: "Patel: AI chips will consume 86% of N3 wafer output by 2027 — Rubin gets delayed if Apple wins the allocation fight" },
    amd:              { dir: "down",  note: "Patel: AMD is running at single-digit GPU share — a wafer crunch exposes how little TSMC leverage they actually have" },
    ...
    apple:            { dir: "mixed", note: "Baker: TSMC sold out its Arizona fab almost immediately and raised prices 20% — Apple may win the allocation but pays a significant premium" },
    ...
    asml:             { dir: "up",    note: "Patel: ASML makes 70-80 EUV tools per year — the N3 crunch validates that every unit is worth far more than the price" },
    amat:             { dir: "up",    note: "Baker: the best teams at TSMC are focused on N3 — Applied Materials process tools are in high demand at that node" },
  ...
  "euv-ban": {
    asml:             { dir: "down",  note: "Patel: ASML is capped at roughly 100 EUV tools per year by the end of the decade — a ban makes every remaining unit far more contested" },
    ...
    tsmc:             { dir: "down",  note: "Baker: TSMC's N2/A16 capacity expansion is the single most important wafer variable — it freezes without new EUV tools" },
    ...
    intel:            { dir: "down",  note: "Circuit: 18A gives Intel a lifeline; an EUV ban directly blocks the 14A follow-on node that is supposed to save them" },
    skhynix:          { dir: "down",  note: "Baker: HBM costs more than the TSMC node in Nvidia's bill of materials — an EUV ban freezes the HBM4 capacity growth that sustains that premium" },
    micron:           { dir: "down",  note: "Patel: Micron is already behind on HBM4 — an EUV ban makes the gap permanent with no path to close it" },
    ...
    broadcom:         { dir: "down",  note: "TPU v7+ depends on N2; Circuit: Broadcom is guiding to $100B in AI revenue by 2027 — that number assumes the N2 ramp happens" },
    apple:            { dir: "down",  note: "A19/M5 are both N2-targeted — iPhone launch timing is at risk; Baker: Arizona fabs are approaching the bleeding edge" },
  },
  "power-constraint": {
    nvidia:           { dir: "mixed", note: "Baker: power is the primary constraint on AI infrastructure growth for at least five years — demand is unaffected, but deployment timing slips" },
    ...
    arista:           { dir: "down",  note: "Circuit: Nvidia is already the largest networking vendor — switch deployments are tied to rack build timelines that slip with power" },
    microsoft:        { dir: "down",  note: "Baker: multiple US states are actively trying to make new data center construction illegal — Azure expansion is directly at risk" },
    google:           { dir: "down",  note: "Baker: capitalism is working hard on the power problem but it takes 18-24 months to solve — GCP data center builds extend" },
    amazon:           { dir: "down",  note: "Patel: AWS plans to build more capacity than anyone else in 2025-2027 — power availability is the binding constraint on that plan" },
    meta:             { dir: "down",  note: "Baker: the power shortage starts easing around 2027-2028 — Meta's Louisiana and Texas builds face multi-year waits before then" },
    openai:           { dir: "down",  note: "Patel: Stargate is targeting gigawatt-scale sites — power permitting adds 18+ months to every gigawatt of the buildout" },
    ...
    xai:              { dir: "down",  note: "Baker: xAI built Colossus with remarkable speed but relied on external generators — that approach is a long-term liability" },
  },
  "optical-transition": {
    coherent:         { dir: "up",    note: "Circuit: the 800G datacenter interconnect refresh is a multi-year wave — Coherent supplies the transceiver hardware that makes it happen" },
    ...
    nvidia:           { dir: "mixed", note: "Circuit: Nvidia is already the largest networking vendor — it needs to invest in photonics, which is near-term cost but long-term moat" },
    broadcom:         { dir: "up",    note: "Circuit: Broadcom is guiding to $100B in AI revenue by 2027 — co-packaged optics is central to how they get there" },
    ...
    arista:           { dir: "up",    note: "Circuit: Ethernet wins again at each generation — the 800G optical switch refresh is a multi-year revenue wave for Arista" },
    ...
    amazon:           { dir: "up",    note: "Baker: Trainium is the most credible challenger to Nvidia — Trainium 3's optical design is a key part of that cost advantage" },
    meta:             { dir: "up",    note: "Optical scale-out lowers MTIA cluster cost; Circuit: 2026 is the year of custom silicon, validating Meta's bet" },
  },
  "custom-asic-inflection": {
    broadcom:         { dir: "up",    note: "Baker: Broadcom is the go-to ASIC supplier for the hyperscalers — the OpenAI XPU deal could be the largest customer relationship in their history" },
    marvell:          { dir: "up",    note: "Baker: Trainium and Marvell custom silicon are the real challengers to Nvidia — ramps accelerating, estimates getting beaten" },
    tsmc:             { dir: "up",    note: "Baker: TSMC still fabs all the custom ASICs — same leading-edge wafer demand, but now spread across a more diverse customer base" },
    arm:              { dir: "up",    note: "Circuit: ARM was going parabolic on the agentic CPU thesis — every custom ASIC uses Arm ISA, so royalties scale directly with volume" },
    google:           { dir: "up",    note: "Patel: TPUs account for the majority of Google and Anthropic training infrastructure — custom ASIC economics improve versus renting GPUs" },
    amazon:           { dir: "up",    note: "Baker: Trainium in 2026 is what TPUs were to 2025 — the AWS TCO advantage compounds as the ramp continues" },
    microsoft:        { dir: "up",    note: "Maia reduces Nvidia dependency; Patel: the Maia program is central to Microsoft's Azure margin story" },
    meta:             { dir: "up",    note: "Circuit: 2026 is the year of custom silicon — MTIA cuts Nvidia spending and the margin story strengthens" },
    openai:           { dir: "mixed", note: "Baker: he would be surprised if many ASICs beyond Trainium and TPU actually succeed — the OpenAI XPU faces real execution risk" },
    anthropic:        { dir: "up",    note: "Patel: Anthropic committed to more than a million TPU chips; Anthropic choosing TPU over GPU speaks to how compelling the economics are" },
    nvidia:           { dir: "down",  note: "Baker: the real competitive fight is between Nvidia and the Google TPU — inference market share erosion begins as ASICs scale" },
  },
  "token-commoditization": {
    openai:           { dir: "down",  note: "Patel: OpenAI is adding revenue faster than almost any company in history, but pricing pressure from more efficient models compresses margin" },
    anthropic:        { dir: "down",  note: "Patel: Anthropic is roughly four times more capital efficient than OpenAI — but token price collapse hurts every model provider" },
    xai:              { dir: "down",  note: "Baker: xAI has among the lowest cost per token of any frontier lab — but commoditization erodes the ROI thesis for Colossus" },
    nvidia:           { dir: "down",  note: "Baker: if training capex pauses, the entire $2T GPU demand thesis evaporates immediately — token commoditization is the key risk" },
    amd:              { dir: "down",  note: "Patel: AMD is running at single-digit inference share — inference repricing hits AMD hardest with the least room to maneuver" },
    broadcom:         { dir: "up",    note: "Baker: most custom ASICs will fail, but Broadcom's $100B AI revenue guidance implies their specific ASIC partnerships survive" },
    marvell:          { dir: "up",    note: "Custom silicon economics improve when GPU TCO looks expensive in a world of commodity token pricing" },
    microsoft:        { dir: "down",  note: "Azure AI revenue margin is at risk; Patel: OpenAI and Microsoft revenue are both highly sensitive to token price declines" },
    google:           { dir: "mixed", note: "Baker: Google is never not going to be in a strong position — TPU costs drop, but Gemini inference revenue margin gets squeezed too" },
    amazon:           { dir: "mixed", note: "Baker: Trainium looks prescient in this environment — AWS AI pricing must fall, but Trainium TCO protects margin better than GPU alternatives" },
    ...
    coreweave:        { dir: "down",  note: "Patel: H100 rental prices already fell from $4 to $2 per hour — token commoditization accelerates that trend further" },
  },
  "chips-act": {
    intel:            { dir: "up",    note: "Circuit: 18A gives Intel a genuine lifeline — the CHIPS Act $8.5B grant plus $11B in loans de-risks the entire foundry bet" },
    micron:           { dir: "up",    note: "Baker: he doesn't know anyone who isn't bullish on DRAM — the CHIPS Act $6B grant enables US HBM production and the stock re-rates" },
    tsmc:             { dir: "up",    note: "Baker: TSMC opened Arizona, sold out immediately and raised wafer prices 20% — the grant validates their US strategy entirely" },
    ...
    "samsung-foundry":{ dir: "up",    note: "$6B Taylor Texas grant; Baker: the allied supply chain is the long-term strategy and Samsung is a key node in it" },
    ase:              { dir: "up",    note: "Packaging onshoring grants are flowing; Circuit: TSMC CoPAS packaging is coming — US OSATs are well-positioned to benefit" },
    amkor:            { dir: "up",    note: "Co-invested with TSMC in Arizona; Baker: Terafab-style talent clustering is what makes the Arizona ecosystem real" },
    amat:             { dir: "up",    note: "Baker: Applied Materials teams will follow the leading fabs — domestic builds generate a fresh equipment order wave" },
    ...
    klac:             { dir: "up",    note: "Baker: KLA teams follow the leading fabs — every new US fab line requires KLA process control from day one" },
  },
  "arm-disruption": {
    arm:              { dir: "mixed", note: "Circuit: ARM was going parabolic on the agentic CPU thesis with $15B in orders — but a licensing dispute creates real RISC-V migration risk" },
    nvidia:           { dir: "down",  note: "Baker: the Grace CPU is central to Nvidia's data center architecture story — a licensing disruption threatens the whole stack" },
    ...
    tsmc:             { dir: "down",  note: "Baker: TSMC sold out immediately — Arm disruption slows tape-outs by designers pausing decisions, and those chips compete for TSMC slots" },
  ...
  "intel-foundry-comeback": {
    intel:            { dir: "up",    note: "Circuit: 18A gives Intel a real lifeline; if 14A delivers, Intel re-enters the process technology competition with a credible roadmap" },
    microsoft:        { dir: "up",    note: "Circuit: Microsoft is the first major external 18A customer — chip costs fall and Maia silicon optionality expands meaningfully" },
    apple:            { dir: "up",    note: "Baker: TSMC raised prices 20% after selling out — Apple getting second-source options at Intel reduces TSMC pricing leverage" },
    asml:             { dir: "up",    note: "Patel: ASML makes 70-80 EUV tools per year — Intel 18A being the first High-NA commercial validation drives the next tool order cycle" },
    amat:             { dir: "up",    note: "Baker: Applied Materials' backside power delivery technology is key to 18A — a direct process technology win for them" },
    klac:             { dir: "up",    note: "Circuit: Intel posted a Q1 2026 earnings beat — an 18A ramp requires a full KLA metrology refresh at every process step" },
    arm:              { dir: "up",    note: "Circuit: ARM was going parabolic on the agentic CPU thesis — 18A customer chips all use Arm ISA, adding royalty volume" },
    snps:             { dir: "up",    note: "18A PDK is validated in Synopsys tools — more tape-out activity at Intel foundry drives EDA revenue" },
    cdns:             { dir: "up",    note: "Circuit: Intel foundry was not abandoned — Cadence 18A design tools are validated and customer design wins are following" },
  },
  "memory-oversupply": {
    skhynix:          { dir: "down",  note: "Baker: DRAM companies already trade at mid-single digit multiples — oversupply removes the one remaining premium, which is HBM scarcity" },
    micron:           { dir: "down",  note: "Baker: he doesn't know anyone who isn't bullish on DRAM long-term — but an oversupply scenario is exactly the bear case that would break that thesis" },
    "samsung-memory": { dir: "mixed", note: "Samsung uses oversupply as a strategic pricing weapon against rivals; Circuit: memory margins were in the eighties — both dynamics are at risk" },
    ...
    lrcx:             { dir: "down",  note: "Circuit: the memory run was the key driver of Lam — oversupply means memory capex cuts hit Lam Research the hardest" },
    ...
    nvidia:           { dir: "up",    note: "Baker: HBM costs more than the TSMC node in Nvidia's bill of materials — cheaper HBM from oversupply would improve GPU gross margins" },
  },
  "asml-shock": {
    asml:             { dir: "down",  note: "Patel: ASML makes roughly 70 EUV tools per year and is targeting 80 — a delay exposes how thin the production margin actually is" },
    ...
    tsmc:             { dir: "down",  note: "Baker: TSMC capacity decisions are the most important variable — without EUV tool deliveries, N2/A16 expansion freezes" },
    ...
    intel:            { dir: "down",  note: "Circuit: 18A buys Intel time; an ASML shock directly blocks the 14A follow-on node that is supposed to make the comeback sustainable" },
    skhynix:          { dir: "down",  note: "Baker: HBM costs more than the TSMC node in Nvidia's bill of materials — HBM4 capacity growth freezes without EUV tool access" },
    micron:           { dir: "down",  note: "Patel: Micron is already out of the picture for Rubin HBM4 — an ASML delay makes that gap permanent" },
    nvidia:           { dir: "down",  note: "Baker: Rubin depends on TSMC's N2 node — an ASML shock means no N2, and the Rubin roadmap slips with no alternative" },
    amd:              { dir: "down",  note: "Patel: AMD is already running behind — next-gen MI-series is N2-targeted, and an ASML delay extends the gap versus Nvidia further" },
    apple:            { dir: "down",  note: "A19/M5 are both N2-targeted; Baker: TSMC Arizona is approaching the bleeding edge — an ASML shock propagates to the Apple product roadmap" },
  },
  "ai-capex-supercycle": {
    nvidia:           { dir: "up",    note: "Baker: if TSMC expands capacity, Nvidia could sell $2T worth of GPUs in 2026-2027 — the supercycle is the scenario that proves it" },
    skhynix:          { dir: "up",    note: "Baker: HBM costs more than the TSMC node in Nvidia's bill of materials — HBM demand doubling means SK Hynix re-rates significantly" },
    micron:           { dir: "up",    note: "Baker: he doesn't know anyone who isn't bullish on DRAM — a full capex supercycle is the scenario that closes the multiple gap" },
    tsmc:             { dir: "up",    note: "Baker: TSMC capacity decisions are the most important variable — a supercycle proves they were right to invest aggressively" },
    broadcom:         { dir: "up",    note: "Circuit: Broadcom is guiding to $100B in AI revenue by 2027 — the supercycle is exactly the environment that gets them there" },
    marvell:          { dir: "up",    note: "Baker: Trainium in 2026 is what TPUs were to 2025 — the supercycle validates the entire custom silicon thesis" },
    ...
    supermicro:       { dir: "up",    note: "Circuit: Nvidia essentially created the neocloud industry — AI server assembly demand doubles in a supercycle" },
    ...
    arista:           { dir: "up",    note: "Circuit: Nvidia is already the largest networking vendor — the 800G port refresh cycle accelerates through the supercycle" },
    ...
    microsoft:        { dir: "up",    note: "Patel: Azure is dwarfing rivals on AI capex — the supercycle justifies every dollar of the Stargate commitment" },
    google:           { dir: "up",    note: "Patel: the TPUv7 Ironwood deal is worth roughly $52B — GCP AI revenue inflects in a full supercycle environment" },
    amazon:           { dir: "up",    note: "Patel: AWS plans to build more capacity than anyone else in 2025-2027 — the supercycle fully vindicates that decision" },
    meta:             { dir: "up",    note: "Llama training at scale; Circuit: 2026 is the year of custom silicon — Meta's MTIA bet finally pays off in this environment" },
    oracle:           { dir: "up",    note: "Patel: Stargate is a $300B deal at $10-15B per gigawatt — the supercycle re-rates Oracle as a core AI infrastructure company" },
    coreweave:        { dir: "up",    note: "Circuit: Nvidia created the neocloud market — pure-play GPU cloud revenue doubles and the IPO timing looks prescient" },
  },
  "openai-demand-shock": {
    openai:           { dir: "up",    note: "Patel: OpenAI is adding revenue faster than almost any company in history — the demand shock accelerates the path to IPO" },
    microsoft:        { dir: "up",    note: "Patel: Azure is dwarfing rivals on AI capex — the $500B Stargate commitment is fully justified by OpenAI demand alone" },
    oracle:           { dir: "up",    note: "Patel: Stargate is a $300B deal over five years — OCI becomes the primary non-Azure host for OpenAI and the stock re-rates" },
    coreweave:        { dir: "up",    note: "Circuit: OpenAI as the anchor customer validates the entire neocloud model that Nvidia essentially built" },
    nvidia:           { dir: "up",    note: "Baker: if TSMC expanded capacity, Nvidia could sell $2T of GPUs — OpenAI is the single demand event that proves that thesis" },
    tsmc:             { dir: "up",    note: "Baker: TSMC capacity is the most important variable — OpenAI GPU wafer demand is a meaningful allocation event at the leading edge" },
    skhynix:          { dir: "up",    note: "Baker: HBM costs more than the TSMC node in Nvidia's bill of materials — an OpenAI GPU surge translates into a multi-billion dollar SK Hynix contract" },
    broadcom:         { dir: "up",    note: "Baker: the OpenAI XPU ASIC deal with Broadcom is essentially a windfall for them — could be their largest customer relationship ever" },
    supermicro:       { dir: "up",    note: "Circuit: the neocloud ecosystem — AI rack assembly for Stargate creates over $1B in revenue concentration" },
    foxconn:          { dir: "up",    note: "GB200 rack manufacturing for Stargate; Baker: hardware engineering at Terafab scale is the model for what comes next" },
    arista:           { dir: "up",    note: "Circuit: Nvidia is already the largest networking vendor — the Stargate data center build requires hundreds of thousands of switch ports" },
  },
  "hbm4-transition": {
    skhynix:          { dir: "up",    note: "Patel: Samsung is effectively out of the picture for Rubin HBM4 — SK Hynix becomes the sole qualified supplier and holds maximum pricing power" },
    micron:           { dir: "down",  note: "Patel: Micron is well behind and effectively out of the picture for Rubin HBM4 — the company misses the most important GPU generation cycle" },
    "samsung-memory": { dir: "down",  note: "Patel: Samsung was already flagged as out for Rubin HBM4 — the HBM3E qualification lag persists into the next generation" },
    lrcx:             { dir: "up",    note: "Circuit: the memory run keeps going — HBM4 stacking requires more advanced etch steps, and Lam Research orders accelerate" },
    nvidia:           { dir: "mixed", note: "Baker: HBM costs more than the TSMC node in Nvidia's bill of materials — chips ship, but sole-sourcing from SK Hynix carries a supply risk premium" },
    amd:              { dir: "down",  note: "Patel: AMD is running pretty mid — MI400 gets delayed if HBM4 supply is concentrated entirely at SK Hynix" },
    openai:           { dir: "down",  note: "Patel: OpenAI is targeting 5-10GW of compute — any GPU delivery timing risk delays every gigawatt in the plan" },
    xai:              { dir: "down",  note: "Baker: xAI gets GPU access ahead of most other customers — an HBM4 constraint narrows that structural timing advantage" },
    coreweave:        { dir: "down",  note: "Circuit: the neocloud model requires continuous GPU additions — HBM4 shortage delays next-gen GPU capacity for months" },
  },
  "geopolitical-japan-korea": {
    "shin-etsu":      { dir: "down",  note: "Baker: the allied supply chain coordination is the long-term strategy — Japan alignment restricts China wafer sales for Shin-Etsu" },
    ...
    skhynix:          { dir: "down",  note: "Patel: HBM exports to Chinese AI customers are under increasing scrutiny — SK Hynix is the most exposed Korean memory company" },
    "samsung-foundry":{ dir: "mixed", note: "Baker: China's fab technology is four years behind and the gap is growing — Samsung gains share versus China but still faces export review uncertainty" },
    "samsung-memory": { dir: "down",  note: "HBM exports to China are increasingly restricted; Patel: the memory supercycle helps, but the China revenue cut is a real headwind" },
    tsmc:             { dir: "up",    note: "Baker: the allied supply chain coordination benefits Taiwan — competitor access to advanced tools gets cut while TSMC gains relative position" },
    asml:             { dir: "down",  note: "Patel: ASML is already capped at 70-80 EUV tools per year — a DUV ban removes the incremental China volume that was still flowing" },
  },
  "packaging-unbundled": {
    tsmc:             { dir: "down",  note: "Patel: CoWoS is the actual gating constraint on the Blackwell ramp — packaging competition from OSATs erodes that pricing power" },
    ase:              { dir: "up",    note: "Circuit: TSMC's own CoPAS packaging offering isn't coming until 2028 — ASE gains meaningful AI chip packaging share in the gap" },
    amkor:            { dir: "up",    note: "Amkor is co-invested with TSMC in Arizona; Baker: Terafab-style talent clustering makes the US packaging ecosystem real" },
    nvidia:           { dir: "up",    note: "Patel: CoWoS was the Blackwell bottleneck — packaging competition from OSATs lowers that cost and improves GPU economics" },
    amd:              { dir: "up",    note: "Multiple packaging sources reduce supply risk; Patel: AMD needs every cost and supply advantage it can get versus Nvidia" },
    broadcom:         { dir: "up",    note: "Circuit: Broadcom is guiding to $100B in AI revenue by 2027 — packaging optionality improves the economics of TPU and XPU programs" },
    "samsung-foundry":{ dir: "up",    note: "Baker: Samsung is a key node in the allied supply chain — a fab plus packaging bundle becomes more competitive against TSMC's offering" },
    intel:            { dir: "up",    note: "Circuit: 18A keeps Intel's foundry alive — EMIB and Foveros advanced packaging gain credibility as real alternatives to TSMC CoWoS" },
  },
};

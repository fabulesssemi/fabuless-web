/**
 * Retrieval pipeline for The Circuit Lens.
 * Same architecture as Baker/Dylan Patel Lens — hits circuit_chunks table.
 *
 * Flow:
...
async function rewriteQuery(userQuestion: string, history: ConversationTurn[] = []): Promise<string[]> {
  ...
  const response = await anthropic.messages.create({
    ...
    messages: [{
      role: "user",
      content: `You are a search query optimizer for a semiconductor and chip industry research database based on The Circuit podcast.

${contextSummary}Rewrite this user question into exactly 3 search queries optimized for retrieving relevant podcast transcript chunks. If the question is vague (e.g. "explain more", "go deeper"), use the conversation context to infer the specific topic. Each query should target a different angle.

User question: "${userQuestion}"

Respond with ONLY a JSON array of 3 strings. No explanation.
Example: ["Marvell earnings beat data center revenue", "custom silicon hyperscaler demand ASIC", "semiconductor supply chain packaging constraint CoWoS"]`,
    }],
  });
...
async function vectorSearch(queryEmbedding: number[], opts: RetrievalOptions): Promise<any[]> {
  const { data } = await getSupabase().rpc("match_circuit_chunks", {
...
async function keywordSearch(queryText: string, opts: RetrievalOptions): Promise<any[]> {
  const { data } = await getSupabase().rpc("keyword_search_circuit_chunks", {

...
function parseAnswerTier(answer: string): AnswerTier {
  ...
  if (upper.startsWith("**PATEL LENS INFERENCE**") || upper.startsWith("PATEL LENS INFERENCE")) return "inference";
...
export async function queryDylanPatelLens(
  ...
  const response = await anthropic.messages.create({
    ...
    max_tokens: 2048,
    system: [{ type: "text", text: DYLAN_PATEL_LENS_SYSTEM_PROMPT, cache_control: { type: "ephemeral" } }],
    messages,
  } as any);
